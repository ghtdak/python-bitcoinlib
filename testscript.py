...
for height in xrange(chaindb.getheight()):
    ...
    log.write("Scanning block #%d %064x (%d tx)" %
              (height, block.sha256, len(block.vtx)))
    ...
    scanned += 1
    if (scanned % 1000) == 0:
        print "Scanned %d blocks (%d warnings, %d failures)" % (
            scanned, warnings, failures)

print "Scanned %d blocks (%d warnings, %d failures)" % (scanned, warnings,
                                                        failures)
