...
class ChainDb(object):
    ...
    def __init__(self,
        ...
        self.blk_write = io.BufferedWriter(io.FileIO(datadir + '/blocks.dat',
                                                     'ab'))
        self.blk_read = io.BufferedReader(io.FileIO(datadir + '/blocks.dat',
                                                    'rb'))
        ...
        for i in xrange(len(tx.vin)):
            ...
            if txfrom is None and check_mempool:
                try:
                    ...
                    return False
            if txfrom is None:
                self.log.write("TX %064x/%d no-dep %064x" % (tx.sha256, i,
                                                             txin.prevout.hash))
                return False
    ...
    def reorganize(self, new_best_blkhash):
        ...
        for block in conn:
            if not self.connect_block(
                    ser_uint256(block.sha256), block,
                    self.getblockmeta(block.sha256)):
                return False
    ...
    def set_best_chain(self, ser_prevhash, ser_hash, block, blkmeta):
        ...
        if (blkmeta.height == 0 or self.db.Get('misc:tophash') == ser_prevhash):
