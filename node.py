...
class NodeConn(Greenlet):
    ...
    def got_message(self, message):
        ...
        if message.command == "version":
            ...
            if (len(message.inv) == 1 and message.inv[0].type == MSG_BLOCK and
            ...
            if not self.chaindb.tx_connected(message.tx):
                ...
                self.log.write("MemPool: Ignoring failed-sig TX %064x" %
                               (message.tx.sha256,))
            else:
...
class PeerManager(object):
    ...
    def add(self, host, port):
        ...
        c = NodeConn(host, port, self.log, self, self.mempool, self.chaindb,
                     self.netmagic)
        self.peers.append(c)
        return c
    ...
    def new_addrs(self, addrs):
        ...
        self.log.write(
            "PeerManager: Received %d new addresses (%d addrs, %d tried)" % (
...
if __name__ == '__main__':
    ...
    chaindb = ChainDb.ChainDb(settings['db'], log, mempool, netmagic, False,
                              False)
    ...
    rpcexec = rpc.RPCExec(peermgr, mempool, chaindb, log, settings['rpcuser'],
                          settings['rpcpass'])
