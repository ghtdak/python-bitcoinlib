...
_ssl.BN_mod_mul.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
                            ctypes.c_void_p, ctypes.c_void_p]

_ssl.BN_mod_sub.restype = ctypes.c_int
_ssl.BN_mod_sub.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
                            ctypes.c_void_p, ctypes.c_void_p]
...
_ssl.EC_GROUP_get_curve_GFp.argtypes = [ctypes.c_void_p, ctypes.c_void_p,
                                        ctypes.c_void_p, ctypes.c_void_p,
                                        ctypes.c_void_p]
...
_ssl.EC_GROUP_get_order.argtypes = [ctypes.c_void_p, ctypes.c_void_p,
                                    ctypes.c_void_p]
...
_ssl.EC_POINT_mul.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
                              ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]

_ssl.EC_POINT_set_compressed_coordinates_GFp.restype = ctypes.c_int
_ssl.EC_POINT_set_compressed_coordinates_GFp.argtypes = [ctypes.c_void_p,
                                                         ctypes.c_void_p,
                                                         ctypes.c_void_p,
                                                         ctypes.c_int,
                                                         ctypes.c_void_p]
...
_ssl.ECDSA_sign.argtypes = [ctypes.c_int, ctypes.c_void_p, ctypes.c_int,
                            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
...
_ssl.ECDSA_verify.argtypes = [ctypes.c_int, ctypes.c_void_p, ctypes.c_int,
...
_ssl.ECDH_compute_key.argtypes = [ctypes.c_void_p, ctypes.c_int,
                                  ctypes.c_void_p, ctypes.c_void_p]

_ssl.ERR_error_string_n.restype = None
_ssl.ERR_error_string_n.argtypes = [ctypes.c_ulong, ctypes.c_char_p,
                                    ctypes.c_size_t]
...
_ssl.d2i_ECPrivateKey.argtypes = [ctypes.c_void_p, ctypes.c_void_p,
                                  ctypes.c_long]
...
class CECKey:
    ...
    def get_raw_ecdh_key(self, other_pubkey):
        ...
        r = _ssl.ECDH_compute_key(
            ctypes.pointer(ecdh_keybuffer), 32,
            _ssl.EC_KEY_get0_public_key(other_pubkey.k), self.k, 0)
        if r != 32:
            raise Exception('CKey.get_ecdh_key(): ECDH_compute_key() failed')
        return ecdh_keybuffer.raw

    def get_ecdh_key(self,
                     other_pubkey,
    ...
    def sign(self, _hash):  # pylint: disable=redefined-builtin
        ...
        result = _ssl.ECDSA_sign(0, _hash, len(_hash), mb_sig,
    ...
    def verify(self, _hash, sig):  # pylint: disable=redefined-builtin
        ...
        _ssl.d2i_ECDSA_SIG(
            ctypes.byref(norm_sig), ctypes.byref(ctypes.c_char_p(sig)),
            len(sig))
        ...
        return _ssl.ECDSA_verify(0, _hash, len(_hash), norm_der, derlen,
                                 self.k) == 1
    ...
    def recover(self, sigR, sigS, msg, msglen, recid, check):
        ...
        try:
            ...
            if R is None:
                return -2
            if not _ssl.EC_POINT_set_compressed_coordinates_GFp(group, R, x,
                                                                recid % 2, ctx):
                return 0
...
class CPubKey(bytes):
    ...
    def recover_compact(cls, _hash, sig):  # pylint: disable=redefined-builtin
        ...
        if len(sig) != 65:
            raise ValueError("Signature should be 65 characters, not [%d]" %
                             (len(sig),))
    ...
    def __repr__(self):
        ...
        # change for py2/3
        if sys.version > '3':
            return '%s(%s)' % (self.__class__.__name__, super(CPubKey,
                                                              self).__repr__())
        else:
