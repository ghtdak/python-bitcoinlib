...
class CScriptOp(int):
    ...
    def encode_op_n(n):
        ...
        if not (0 <= n <= 16):
            raise ValueError('Integer must be in range 0 <= n <= 16, got %d' %
                             n)
...
class CScript(bytes):
    ...
    def __add__(self, other):
        ...
        try:
            ...
            raise TypeError('Can not add a %r instance to a CScript' %
                            other.__class__)
    ...
    def __new__(cls, value=b''):
        if isinstance(value, bytes) or isinstance(value, bytearray):
            ...
            return super(CScript, cls).__new__(cls,
                                               b''.join(coerce_iterable(value)))
...
def IsLowDERSignature(sig):
    ...
    s_val = list(struct.unpack(
        str(length_s) + 'B', sig[6 + length_r:6 + length_r + length_s]))
