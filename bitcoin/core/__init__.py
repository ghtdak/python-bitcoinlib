...
class CTxIn(ImmutableSerializable):
    ...
    def from_txin(cls, txin):
        ...
        if txin.__class__ is CTxIn:
            ...
            return cls(
                COutPoint.from_txout(txin.prevout), txin.scriptSig,
                txin.nSequence)
...
class CTxOut(ImmutableSerializable):
    ...
    def __repr__(self):
        if self.nValue >= 0:
            return "CTxOut(%s*COIN, %r)" % (str_money_value(self.nValue),
                                            self.scriptPubKey)
        else:
...
class CTransaction(ImmutableSerializable):
    ...
    def __init__(self, vin=(), vout=(), nLockTime=0, nVersion=1):
        ...
        object.__setattr__(self, 'vin', tuple(CTxIn.from_txin(txin)
                                              for txin in vin))
        object.__setattr__(self, 'vout', tuple(CTxOut.from_txout(txout)
                                               for txout in vout))
    ...
    def __repr__(self):
        return "CTransaction(%r, %r, %i, %i)" % (self.vin, self.vout,
                                                 self.nLockTime, self.nVersion)
...
class CBlockHeader(ImmutableSerializable):
    """A block header"""
    __slots__ = ['nVersion', 'hashPrevBlock', 'hashMerkleRoot', 'nTime',
                 'nBits', 'nNonce']
    ...
    def stream_deserialize(cls, f):
        ...
        return cls(nVersion, hashPrevBlock, hashMerkleRoot, nTime, nBits,
                   nNonce)
...
class CBlock(CBlockHeader):
    ...
    def build_merkle_tree_from_txids(txids):
        ...
        while size > 1:
            for i in range(0, size, 2):
                i2 = min(i + 1, size - 1)
                merkle_tree.append(Hash(merkle_tree[j + i] + merkle_tree[j +
                                                                         i2]))
    ...
    def __init__(self,
                 ...
                 vtx=()):
        """Create a new block"""
        super(CBlock, self).__init__(nVersion, hashPrevBlock, hashMerkleRoot,
                                     nTime, nBits, nNonce)
