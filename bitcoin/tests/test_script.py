...
class Test_CScript(unittest.TestCase):
    ...
    def test_tokenize_roundtrip(self):
        ...
        T(b'fafbfcfd', [CScriptOp(0xfa), CScriptOp(0xfb), CScriptOp(0xfc),
                        CScriptOp(0xfd)])
        ...
        T(b'512103e2a0e6a91fa985ce4dda7f048fca5ec8264292aed9290594321aa53d37fdea32410478d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0b6be9ab35c71a1518063243acd4dfe96b66e3f2ec8013c8e072cd09b3834a19f81f659cc345552ae',
          [1, unhexlify(
              b'03e2a0e6a91fa985ce4dda7f048fca5ec8264292aed9290594321aa53d37fdea32'),
           unhexlify(
               b'0478d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0b6be9ab35c71a1518063243acd4dfe96b66e3f2ec8013c8e072cd09b3834a19f81f659cc3455'),
           2, OP_CHECKMULTISIG])
    ...
    def test_repr(self):
        ...
        T(
            CScript([1, b'z\xc9w\xd87=\xf8u\xec\xed\xa3b)\x8e]\t\xd4\xb7+S',
                     OP_DROP]),
            "CScript([1, b'z\\xc9w\\xd87=\\xf8u\\xec\\xed\\xa3b)\\x8e]\\t\\xd4\\xb7+S', OP_DROP])")

        T(
            CScript(unhexlify(b'0001ff515261ff')),
            "CScript([b'', b'\\xff', 1, 2, OP_NOP, OP_INVALIDOPCODE])")

        # truncated scripts
        T(
            CScript(unhexlify(b'6101')),
        ...
        T(
            CScript(unhexlify(b'614c')),
            "CScript([OP_NOP, <ERROR: PUSHDATA1: missing data length>])")

        T(
            CScript(unhexlify(b'614c0200')),
