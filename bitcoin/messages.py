...
class msg_getblocks(MsgSerializable):
    ...
    def __repr__(self):
        return "msg_getblocks(locator=%s hashstop=%s)" % (repr(self.locator),
                                                          b2x(self.hashstop))
...
class msg_getheaders(MsgSerializable):
    ...
    def __repr__(self):
        return "msg_getheaders(locator=%s hashstop=%s)" % (repr(self.locator),
                                                           b2x(self.hashstop))
...
class msg_reject(MsgSerializable):
    ...
    def __repr__(self):
        return "msg_reject(messsage=%s, ccode=%s, reason=%s)" % (
            self.message, self.ccode, self.reason)
...
msg_classes = [msg_version, msg_verack, msg_addr, msg_alert, msg_inv,
               ...
               msg_headers, msg_tx, msg_block, msg_getaddr, msg_ping, msg_pong,
               msg_reject, msg_mempool]
