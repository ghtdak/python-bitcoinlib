...
class BaseProxy(object):
    ...
    def __init__(self,
                ...
                if conf['rpcssl'] and 'rpcsslcertificatechainfile' in conf and 'rpcsslprivatekeyfile' in conf:
                    ...
                    if os.path.exists(conf['rpcsslcertificatechainfile']):
                        certificate = conf['rpcsslcertificatechainfile']
                            ...
                            os.path.dirname(btc_conf_file), conf[
                                'rpcsslcertificatechainfile'])):
                        certificate = os.path.join(
                            os.path.dirname(btc_conf_file),
                            conf['rpcsslcertificatechainfile'])
                    else:
                        raise ValueError(
                            'The value of rpcsslcertificatechainfile is not correctly specified in the configuration file: %s'
                            % btc_conf_file)
                    if os.path.exists(conf['rpcsslprivatekeyfile']):
                        ...
                        private_key = os.path.join(
                            os.path.dirname(btc_conf_file),
                            conf['rpcsslprivatekeyfile'])
                    else:
                        raise ValueError(
                            'The value of rpcsslprivatekeyfile is not correctly specified in the configuration file: %s'
                            % btc_conf_file)
        ...
        if self.__url.scheme == 'https':
            ...
            self.__conn = httplib.HTTPConnection(self.__url.hostname,
                                                 port=port,
                                                 timeout=timeout)
...
class Proxy(BaseProxy):
    ...
    def lockunspent(self, unlock, outpoints):
        ...
        json_outpoints = [{'txid': b2lx(outpoint.hash),
                           'vout': outpoint.n} for outpoint in outpoints]
