...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        ...
        self.pub = bitcoin.core.key.CPubKey(self._cec_key.get_pubkey(),
                                            self._cec_key)
    ...
    def __repr__(self):
        ...
        # change for py2/3
        if sys.version > '3':
            return '%s(%s, %r)' % (self.__class__.__name__, super(
                CKey, self).__repr__(), self.is_compressed)
        else:
            return '%s(b%s, %r)' % (self.__class__.__name__, super(
                CKey, self).__repr__(), self.is_compressed)
